...
# Select and prepare data for clustering
# Focus on vocal behavior patterns
clustering_data <- raw_data %>%
  # Select main behavioral scores (not binary indicators)
  select(
    barky, howly, growly, whiney, woofy,  # Main vocal tendency scores
    bark_sep_fr, howl_sep, howl_soc,      # Behavioral pattern scores
    growl_food, growl_toy, growl_bone,     # Specific behavior scores
    growl_dog, growl_fam, howl_str         # Additional pattern scores
  ) %>%
...
n_samples <- nrow(clustering_data)
m_size <- min(floor(n_samples/10), n_samples - 1)  # Ensure m is valid
hopkins_result <- hopkins(scale(clustering_data), m = m_size)

# 2. Visual Assessment of Cluster Tendency (VAT)
vat_plot <- fviz_dist(dist(scale(clustering_data)), show_labels = FALSE) +
...
wss <- numeric(10)
for (i in 1:10) {
  km <- kmeans(scale(clustering_data), centers = i, nstart = 25)
...
silhouette_plot <- fviz_nbclust(scale(clustering_data), kmeans, method = "silhouette") +
...
paste(names(clustering_data), collapse = ", "),
hopkins_result,
