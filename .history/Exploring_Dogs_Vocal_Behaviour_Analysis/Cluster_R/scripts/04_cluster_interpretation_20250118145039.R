...
cluster_proportions <- cluster_data %>%
  group_by(Cluster) %>%
...
colnames(cluster_proportions) <- c("Cluster1", "Cluster2")
cluster_proportions$Variable <- rownames(cluster_proportions)
...
describe_cluster <- function(cluster_num) {
  ...
  top_chars <- cluster_proportions[order(cluster_proportions[paste0("Cluster", cluster_num)], 
                                       decreasing = TRUE), ]
  ...
  for(i in 1:5) {
    cat(sprintf("%d. %s (%.1f%%)\n", 
                ...
                top_chars[i, paste0("Cluster", cluster_num)] * 100))
  }
  cat("\n")
}
