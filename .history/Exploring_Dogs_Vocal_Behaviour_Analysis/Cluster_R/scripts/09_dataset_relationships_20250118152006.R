...
analyze_dataset_patterns <- function(data, name) {
  # Remove ID column and ensure numeric
  data_clean <- as.data.frame(lapply(data[, !colnames(data) %in% c("ID_full")], 
...
# Create correlation matrix between total responses in each dataset
responses_matrix <- data.frame(
  problems = rowSums(as.data.frame(lapply(problems_data[, !colnames(problems_data) %in% c("ID_full")], 
                                        function(x) as.numeric(as.character(x))))),
  growling = rowSums(as.data.frame(lapply(growl_data[, !colnames(growl_data) %in% c("ID_full")], 
                                        function(x) as.numeric(as.character(x))))),
  howling = rowSums(as.data.frame(lapply(howl_data[, !colnames(howl_data) %in% c("ID_full")], 
                                       function(x) as.numeric(as.character(x))))),
  keeping = rowSums(as.data.frame(lapply(keep_data[, !colnames(keep_data) %in% c("ID_full")], 
                                       function(x) as.numeric(as.character(x)))))
)

correlation_matrix <- cor(responses_matrix)
