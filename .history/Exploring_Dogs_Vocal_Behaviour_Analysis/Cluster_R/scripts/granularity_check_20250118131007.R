...
plot1 <- ggplot(gather(grow_to_whom, key = "Target", value = "Growl", -ID_full), aes(x = Target, y = Growl)) +
  ...
  ylab("Count") +
  xlab("Target")
ggsave(filename = file.path(output_folder, "02_granularity_check_grow_to_whom.png"), plot = plot1, width = 8, height = 5)
...
plot2 <- ggplot(gather(howl_on_sound, key = "Sound", value = "Howl", -ID_full), aes(x = Sound, y = Howl)) +
  ...
  ylab("Count") +
  xlab("Sound")
ggsave(filename = file.path(output_folder, "02_granularity_check_howl_on_sound.png"), plot = plot2, width = 8, height = 5)
...
plot3 <- ggplot(gather(keep, key = "Location", value = "Frequency", -ID_full), aes(x = Location, y = Frequency)) +
  ...
  ylab("Count") +
  xlab("Location")
ggsave(filename = file.path(output_folder, "02_granularity_check_keep.png"), plot = plot3, width = 8, height = 5)
...
plot4 <- ggplot(gather(problems, key = "Problem", value = "Occurrence", -ID_full), aes(x = Problem, y = Occurrence)) +
  ...
  ylab("Count") +
  xlab("Problem")
ggsave(filename = file.path(output_folder, "02_granularity_check_problems.png"), plot = plot4, width = 8, height = 5)
