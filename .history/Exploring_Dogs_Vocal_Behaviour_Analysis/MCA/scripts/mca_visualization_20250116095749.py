...
def plot_mca_results(results, filepath, title):
    ...
    # Reconstruct contributions DataFrame
    contrib_data = {}
    for col in contributions['columns']:
        contrib_data[col] = contributions['contributions'][col]
    
    contrib_df = pd.DataFrame.from_dict(contrib_data, orient='index').transpose()
    contrib_df.index = contributions['columns']
    ...
    # Use contribution values for color mapping
    ...
    for i, idx in enumerate(mca_df.index):
        try:
            ...
            point_colors[i] = contributions['contributions'][idx][0]
