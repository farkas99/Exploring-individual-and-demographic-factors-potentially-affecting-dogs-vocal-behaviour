...
def safe_get_contributions(contributions):
    ...
    try:
        # Check if contributions is a dictionary with numeric keys
        if isinstance(contributions.get('contributions'), dict):
            # Assume the first dimension's contributions are most relevant
            first_dim_contrib = contributions['contributions'].get('0', [])
            
            # Use the original columns or create default columns
            columns = contributions.get('columns', list(range(len(first_dim_contrib))))
            
            # Create contribution data
            for i, col in enumerate(columns):
                # Get contributions for each dimension, defaulting to 0 if not found
                dim_contribs = [
                    contributions['contributions'].get(str(dim), [0]*len(columns))[i] 
                    for dim in range(3)  # Assuming 3 dimensions
                ]
                contrib_data[col] = dim_contribs
...
def plot_mca_results(results, filepath, title):
    ...
    # Create heatmap with row labels and color variation
