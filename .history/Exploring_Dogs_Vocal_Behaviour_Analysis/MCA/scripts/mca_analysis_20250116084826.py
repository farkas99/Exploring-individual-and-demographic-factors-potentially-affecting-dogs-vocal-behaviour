...
def plot_mca_results(mca, mca_results, filepath, title, timestamp, original_df):
    ...
    try:
        ...
        contributions = mca.column_contributions_
        point_sizes = contributions['Dim1'].values * 1000  # Scale for visibility
        
        # Color mapping based on contribution
        import matplotlib.cm as cm
        norm = plt.Normalize(point_sizes.min(), point_sizes.max())
        colors = cm.viridis(norm(point_sizes))
        ...
        plt.colorbar(scatter, label='Contribution to Dimension 1')
        ...
        for idx, row in mca_df.iterrows():
            contribution = contributions.loc[idx, 'Dim1']
            plt.annotate(f'{idx}\n({contribution:.2%})', 
                         (row['Dim1'], row['Dim2']), 
                         xytext=(5, 5), 
