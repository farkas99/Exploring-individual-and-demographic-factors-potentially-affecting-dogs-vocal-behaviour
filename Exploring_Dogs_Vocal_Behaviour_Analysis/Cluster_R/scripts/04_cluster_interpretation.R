...
describe_cluster <- function(cluster_num) {
  ...
  sorted_indices <- order(cluster_proportions[[cluster_col]], decreasing = TRUE)
  ...
  for(i in 1:5) {
    cat(sprintf("%d. %s (%.1f%%)\n", 
                i,
                gsub("_", " ", cluster_proportions$Variable[top_indices[i]]),
                cluster_proportions[[cluster_col]][top_indices[i]] * 100))
  }
  cat("\n")
}
